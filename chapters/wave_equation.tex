...
\exercise{1.11}{
	...
	Classically, the peobability of finding particle in range $dx$ is equal to the fraction of time $T$ it takes to get from $a$ to $b$ that it spends on the interval $dx$, and for speed $v(x)$:
	\begin{flalign*}
	...
	\end{flalign*}
		\begin{itemize}
			\item Use conservation of energy to express speeds in terms of potential well and energy.
			\item Find the density for simple harmonic oscillator, $V(x)=\frac{kx^2}{2}$. Check normalization.
			\item Find the first and second expected value of position. Find $\sigma_x$.
		\end{itemize}
}
